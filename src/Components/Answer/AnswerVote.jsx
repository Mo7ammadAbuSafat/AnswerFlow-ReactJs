...
import VoteUsers from "../Question/VoteUsers";
import DoneTwoToneIcon from "@mui/icons-material/DoneTwoTone";

const AnswerVote = ({ answerData }) => {
  ...
  if (
    ...
    initVote = answerData.votes.find((x) => x.user.id === authContext.user.id);
  }
  ...
  const handleVote = async (type) => {
    ...
    await axios
      .post(
        `https://localhost:7127/api/questions/${answerData.questionId}/answers/${answerData.id}/votes`,
        JSON.stringify(data),
        {
          headers: {
  ...
  const handleEditVote = async (type) => {
    ...
    await axios
      .put(
        `https://localhost:7127/api/questions/${answerData.questionId}/answers/${answerData.id}/votes/${userVote.id}`,
        JSON.stringify(data),
        {
          headers: {
            Accept: "application/json",
  ...
  return (
    <Stack sx={{ padding: "3px 0px 3px 0px" }} alignItems={"center"} width={70}>
      ...
      >
        {answerData.finalVotesValue - initVoteValue + userVoteValue}
      </Typography>
      <IconButton
        disabled={!authContext.isLoggedIn || isLoading}
        onClick={() => handleVoteClick(-1)}
      ...
      >
        <VoteUsers votesData={answerData.votes} />
      </PopUpModal>
    </Stack>
  );
};
